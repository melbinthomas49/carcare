...
## 🩺 AI Diagnosis setup (Gemini API — optional but recommended)

The **AI Diagnosis** tab lets users describe symptoms (engine sound, brake squeal,
warning light photo, exhaust smoke photo) and get an AI-generated severity verdict,
cost estimate, and urgency note — powered by Google's Gemini API, called directly
from the browser (no paid backend needed).

**Get a free key:**
1. Go to **aistudio.google.com/apikey** (sign in with any Google account — no credit card)
2. Click **Create API key** → copy it

**Add it to the code:**
1. Open `index-single-file.html`, search for `GEMINI_API_KEY`
2. Replace `"YOUR_GEMINI_API_KEY"` with your real key
3. Save

**⚠️ Security trade-off, stated plainly:** this key is called directly from the
browser, so it's visible to anyone who views your page's source (right-click →
View Page Source). That's the cost of not running a paid backend. To limit the
blast radius if someone copies it:
- In Google AI Studio, set a daily request cap on the key
- Rotate the key occasionally
- Keep an eye on your usage in AI Studio's dashboard

If you leave the placeholder in place, the rest of the app still works fine —
clicking "Get AI Diagnosis" just shows a friendly reminder instead of failing
silently.

**Data model addition:**
```
diagnoses/{autoId}
  ownerId, vehicleId     who it belongs to
  symptoms               array of selected symptom tags
  description             free-text description from the owner
  issue, severity          AI's verdict ("Safe" | "Drive Carefully" | "Stop Driving")
  explanation               plain-language reasoning
  estimatedCostRange        rough INR range
  urgent                    one-line urgency note
  photos                    any attached photos, compressed base64
  createdAt                 timestamp
```
